---
name: biometricrdp-mvc-appkit
description: biometricRDP's architectural contract — every feature lives in an NSViewController that owns its model, its view, AND its HTTP test-API routes. Use this skill on every code-writing turn for the biometricRDP project.
---

# biometricRDP MVC for macOS / AppKit

This skill is **mandatory** for every code change to the biometricRDP
...
- Plain Swift structs / classes in `biometricRDP/<Feature>/<Name>State.swift`.
...
- An `NSView` subclass in `biometricRDP/<Feature>/<Name>View.swift`.
...
- An `NSViewController` subclass in
  `biometricRDP/<Feature>/<Name>Controller.swift`.
...
This is the single most important contract in biometricRDP. Every
...
│   └── biometricRDPWindow.swift                   # NSWindow subclass
│
├── Editor/
...
1. A route registered outside its controller. (e.g. `POST /editor/type`
   declared in `biometricRDP/App/TestAPI/Routes.swift`.)
