...
- **Invariant**: every biometricRDP target ships an explicit `main.swift`
...
- **Invariant**: every `NSTextView` in biometricRDP is built by the canonical
...
- **Invariant**: biometricRDP ships `NSOpenPanel` / `NSSavePanel` first.
...
- **Invariant**: in biometricRDP the build check MUST invoke at least one
...
- **Invariant**: biometricRDP's feature checks talk to the app via
...
- **Invariant**: each agent invocation in biometricRDP is wrapped by Go
...
- **Invariant**: every code-agent invocation in biometricRDP is sandwiched
...
- **Invariant**: in biometricRDP, no quality review starts until the
...
- **Always commit on a working tree.** The old loop refused to
  ...
  biometricRDP's orchestrator `git status --porcelain` checks both
  pre- and post-agent.
